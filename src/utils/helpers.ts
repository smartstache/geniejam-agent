...
// takes the data returned from the helius rpc's custom /v0/token-metadata  call
export function getFirstVerifiedCreator(mintInfo: any) {
  const firstVerifiedCreator = mintInfo.onChainMetadata?.metadata?.data?.creators?.find(creator => creator.verified);
  return firstVerifiedCreator?.address ?? null;
}
